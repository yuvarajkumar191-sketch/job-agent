...
import { Search, Shield, Eye, EyeOff, ExternalLink, Plus, CheckCircle, Clock, Loader2 } from 'lucide-react'
...
export default function JobsPage() {
  ...
  async function startApplication(job: Job) {
    ...
    try {
      ...
      // Open job URL in new tab for manual application
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{total} jobs found &bull; Default filter: score &ge; {minScore}</p>
        </div>
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      {applied.has(job.id) ? (
                        ...
                        <button onClick={() => startApplication(job)} disabled={applying === job.id} className="flex items-center gap-1 px-2 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700 disabled:opacity-50">
                          {applying === job.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <Plus className="w-3 h-3" />}
                          Apply
                        </button>
                      )}
                    </div>
